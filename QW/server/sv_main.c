...
cvar_t sv_delay_in = {"sv_delay_in", "0"};
cvar_t sv_delay_out = {"sv_delay_out", "0"};
...
// get packets
	if (!sv_delay_in.value || (rand() & 0x7fff) / 32768.0 > sv_delay_in.value)
		SV_ReadPackets ();
...
// send messages back to the clients that had packets read this frame
	if (!sv_delay_out.value || (rand() & 0x7fff) / 32768.0 > sv_delay_out.value)
...
void SV_InitLocal (void)
{
	...
	Cvar_RegisterVariable (&sv_delay_in);
	Cvar_RegisterVariable (&sv_delay_out);
