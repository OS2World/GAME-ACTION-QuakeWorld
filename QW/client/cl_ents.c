	...
	for (pnum=0 ; pnum<pack->num_entities ; pnum++)
	{
		...
		for (i=0 ; i<cl_oldnumvisedicts ; i++)
		{
			if (cl_oldvisedicts[i].keynum == ent->keynum)
			{
				...
				for (i = 0; i < 3; i++)
					old_origin[i] += f * (s1->origin[i] - s2->origin[i]);
				break;
			}
		}
...
//Con_DPrintf ("predict: %i\n", msec);
			...
			/*
			if (state->command.forwardmove || state->command.sidemove) {
				Con_Printf("Qpredict %d (%d, %d): (%f %f) -> (%f %f)\n", (int)state->command.msec, (int)state->command.forwardmove, (int)state->command.sidemove, state->origin[0], state->origin[1], exact.origin[0], exact.origin[1]);
			}
			*/
