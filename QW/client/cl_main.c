...
void CL_Ping_f(void)
{
	...
	{
		Con_Printf ("Bad host address\n");
		return;
	}

	if (adr.port == 0)
		adr.port = BigShort (27001);
	sprintf (data, "%c%c%c%c%c\n", 255, 255, 255, 255, A2A_PING);
	NET_SendPacket (strlen(data), data, adr);
}

void CL_Print_f(void)
{
	netadr_t adr;
	char	data[2048];
	char	*host;
	...
	if (Cmd_Argc() != 3)
	{
		Con_Printf ("usage: print <host> <text>\n");
		return;	
	}
	...
	if (adr.port == 0)
		adr.port = BigShort (27001);
	sprintf (data, "%c%c%c%c%c%s\n", 255, 255, 255, 255, A2C_PRINT, Cmd_Argv(2));
	NET_SendPacket (strlen(data), data, adr);
}
...
//
	...
	Cmd_AddCommand ("print", CL_Print_f);
